...
class SMILESRXNMetric(BaseMetric):
    def __init__(self):
        super().__init__(name="SMILES RXN Metric")
...
if __name__ == "__main__":
    compare_obj = CompareObj(
        value_list=[
            "CCO>>CC=O",
            "OCC1OC(OC2(CO)OC(CO)C(O)C2O)C(O)C(O)C1O.O>>OCC1OC(O)C(O)C(O)C1O.OCC1OC(O)(CO)C(O)C1O",
            "invalid_smarts"
        ]
    )
    base_ref = BaseReference(
        value_list=[
            "CCO>>CC=O", 
            "OCC1C(O)C(O)C(O)C(O1)(OC2(C(O))OC(C(O))C(O)C2(O)).O>>OCC1OC(O)C(O)C(O)C1O.OCC1OC(O)(CO)C(O)C1O",
            "CCN>>CC=N"
